...
function employeeUpdater() {
  ...
  for(let i = 0; i < employeesCopy.length; i++) {
      if (employeesCopy[i].firstName === `Theo`) {
        employeesCopy.splice(i,1) 
      }
      if (employeesCopy[i].firstName === `Lorie`) {
        employeesCopy[i].department = `HR` 
      }
  }
